    ...
    internal class Program
    {
        ...
        static void game_menu(Car c, string nick, List<Car> carsList)
        {
            do
            {
                ...
                Console.Clear();
                cars_saver(carsList, nick);
            } while (true);
        }
        static List<Car> car_make(List<Car> carsList, string nick)
        {
            Console.Clear();
            ...
            Console.Write("Podaj ilość paliwa w baku litrach: "); double amaountOfFuel = inputLibrary.Double.restricted_double_input(0, FuelTankCapacity);
        ...
        static void cars_saver(List<Car> carsList, string nick)
        {
            if (!System.IO.Directory.Exists("usersCars/"))
                System.IO.Directory.CreateDirectory("usersCars/");
            List<Car> saveList = new List<Car>(carsList);
            saveList.RemoveAt(0);
            saveList.RemoveAt(0);
            string json = JsonSerializer.Serialize(saveList);
            File.WriteAllText($"usersCars/{nick}Cars.json", json);
        }
        static List<Car> cars_reader(List<Car> carsList, string nick)
        {
